...
.PHONY: all build push build-backend push-backend build-downloader push-downloader build-transfer push-transfer

all: build push

# --- Build ---

build: build-backend build-downloader build-transfer
...
push: push-backend push-downloader push-transfer
...
apply-k8s:
	...
	kubectl apply -f backend/k8s/worker-downloader-go.yaml
	kubectl apply -f backend/k8s/worker-downloader-py.yaml
	kubectl apply -f backend/k8s/worker-transfer-scanner.yaml
	kubectl apply -f backend/k8s/worker-transfer-agent.yaml
