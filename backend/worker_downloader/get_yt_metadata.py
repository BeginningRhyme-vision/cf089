...
def get_cookie_from_backend(machine_name):
    ...
    try:
        ...
        if resp.status_code == 200:
            config = resp.json()
            cookie_content = config.get('cookie_content', '')
            if cookie_content:
                # 处理可能的 JSON 转义字符（如 \n 需要转换为真正的换行符）
                # JSON 序列化时，换行符会被转义为 \n（两个字符），需要转换为真正的换行符
                # 但要注意：如果字符串中真的包含反斜杠+n，也会被替换，这是合理的
                ...
                if not cookie_content.startswith('# Netscape'):
                    # 如果内容包含制表符（Netscape 格式的特征），说明可能是有效的 cookie 数据，只是缺少文件头
                ...
                # 创建临时文件存储 cookie 内容（使用 UTF-8 编码，确保 Netscape 格式正确）
                ...
                # 更新限速器（从配置中）
                parse_rate_limit, download_rate_limit = update_rate_limiter_from_config(config)
                
                # 更新全局配置中的 cookie 文件路径
                COOKIE_CONFIG['cookie_file'] = temp_file.name
                
                # 初始化或更新限速器
                if parse_rate_limit > 0:
                    if parse_rate_limiter is None:
                        parse_rate_limiter = RateLimiter(parse_rate_limit)
                    print(f"从后端获取到 cookie 配置 (机器名: {machine_name}, 解析限流: {parse_rate_limit} requests/min)")
                else:
                    if parse_rate_limiter is not None:
                        parse_rate_limiter.update_rate(0)
                    print(f"从后端获取到 cookie 配置 (机器名: {machine_name}, 无解析限流)")
...
def main():
    ...
    # Cookies 支持：优先从后端获取，然后使用本地 cookie.txt，最后尝试浏览器
    # 1. 优先从后端获取 cookie 配置
    machine_name = get_machine_name()
    cookie_file_from_backend = get_cookie_from_backend(machine_name)
    
    # 2. 本地 cookie.txt 路径：当前目录或脚本目录
    cookie_file_paths = [
        "cookie.txt",
    ...
    if no_cookies:
        ...
        print(f"使用 cookies 文件 (环境变量): {cookies_file_env}")
        # 不指定 player_client，让 yt-dlp 自动选择最佳客户端（类似 --dump-json 的行为）
    elif cookie_file:
        # 使用本地 cookie.txt 文件
