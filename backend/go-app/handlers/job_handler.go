...
type CreateTransferJobRequest struct {
	...
	PeriodicInterval interface{} `json:"periodic_interval"`
}
...
func CreateTransferJob(c *gin.Context) {
	...
	// Handle PeriodicInterval (support string or int/float)
	if req.PeriodicInterval != nil {
		switch v := req.PeriodicInterval.(type) {
		case float64:
			job.PeriodicInterval = int(v)
		case string:
			if iv, err := strconv.Atoi(v); err == nil {
				job.PeriodicInterval = iv
			}
		}
	}
		...
		for _, val := range results {
			...
			if err := json.Unmarshal([]byte(str), &task); err == nil {
				if task.Status == "FAILED" {
					...
					statsKey := fmt.Sprintf("tx:job:%d:stats", task.JobID)
					// Redis keys: [taskKey, statsKey, dirtySetKey, zsetKey, completedHashKey]
					pipe.Eval(ctx, updateTaskScript,
						[]string{taskKey, statsKey, StatsDirtySetTransfer, jobKey, ""},
						data, task.JobID, task.Status, "0", task.ID, "",
					)
		...
		for _, val := range results {
			...
			if err := json.Unmarshal([]byte(str), &task); err == nil {
				...
				if task.Status == "FAILED" && (task.IsDownloadFail || isBotError) {
					...
					completedKey := fmt.Sprintf("job:%d:completed", task.JobID)
					
					// KEYS: [taskKey, statsKey, dirtySetKey, zsetKey, completedHashKey]
					pipe.Eval(ctx, updateTaskScript,
						[]string{taskKey, statsKey, StatsDirtySetYoutube, jobKey, completedKey},
						data, task.JobID, task.Status, "0", task.ID, task.URL,
					)
					
					// Push back to metadata retry queue
					pipe.RPush(ctx, "queue:youtube:metadata_retry", task.ID)
		...
		for _, val := range results {
			...
			if err := json.Unmarshal([]byte(str), &task); err == nil {
				if task.Status == "FAILED" {
					...
					statsKey := fmt.Sprintf("ff:job:%d:stats", task.JobID)
					// KEYS: [taskKey, statsKey, dirtySetKey, zsetKey, completedHashKey]
					pipe.Eval(ctx, updateTaskScript,
						[]string{taskKey, statsKey, StatsDirtySetFfmpeg, jobKey, ""},
						data, task.JobID, task.Status, "0", task.ID, "",
					)
...
type CreateYoutubeJobRequest struct {
	...
	AudioExtension string   `json:"audio_extension" form:"audio_extension"`
	VideoExtension string   `json:"video_extension" form:"video_extension"`
	FilenameTemplate string `json:"filename_template" form:"filename_template"`
...
func CreateYoutubeJob(c *gin.Context) {
	...
	if strings.Contains(contentType, "application/json") {
		...
		req.AudioExtension = c.PostForm("audio_extension")
		req.VideoExtension = c.PostForm("video_extension")
		req.FilenameTemplate = c.PostForm("filename_template")
	...
	job := models.YoutubeJob{
		...
		AudioExtension: req.AudioExtension,
