  ...
  const handleCreate = async () => {
    try {
      ...
      if (fileList.length > 0) {
        ...
        formData.append('audio_extension', values.audio_extension || 'm4a');
        formData.append('video_extension', values.video_extension || 'mp4');
        if (values.filename_template) {
            formData.append('filename_template', values.filename_template);
        }
        ...
        const payload = {
            ...
            audio_extension: values.audio_extension || 'm4a',
            video_extension: values.video_extension || 'mp4',
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item 
            ...
            label="Filename Template (Optional)" 
            help="Supported: %(id), %(title), %(ext), $(date +%Y%m%d). Leave empty for default."
          >
            <Input placeholder="$(date +%Y%m%d)/%(id).%(ext)" />
          </Form.Item>
          <Form.Item 
            name="file_url" 
